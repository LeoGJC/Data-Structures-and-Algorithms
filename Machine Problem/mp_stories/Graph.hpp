...
E & Graph<V,E>::insertEdge(const V & v1, const V & v2) {
  ...
  edgeListIter go = edgeList.begin();
  edgeListIter tmp;
  while (go != edgeList.end()) {
    tmp = go;
    ++go;
  }
  adjList.at(v1.key()).push_back(tmp);
  if (!edgeList.back().get().directed()) {
    adjList.at(v2.key()).push_back(tmp);
  }
  return e;
}
...
void Graph<V,E>::removeEdge(const edgeListIter & it) {
  // TODO: Part 2
  for (auto itt = edgeList.begin(); itt != edgeList.end(); ++itt) {
    if (itt == it) {
      edgeList.remove(itt);
      adjList.at(itt.source().key()).remove(itt);
      if (!edgeList.front().directed()) {
        adjList.at(itt.dest().key()).remove(itt);
      }
    }
  }
}
...
bool Graph<V,E>::isAdjacent(const std::string key1, const std::string key2) const {
  // TODO: Part 2
  list<edgeListIter> go = adjList.at(key1);
  for (auto it : go) {
    if (it->get().dest() == key2) {
      return true;
    }
