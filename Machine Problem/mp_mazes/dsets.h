 ...
 *
 * It is implemented with the optimizations discussed in class, as up-trees stored
 * in a single vector of ints. Specifically, path compression and union-by-size
 * are used. Each place in the vector represents a node.
...
private:
  /**
   * vector of ints whose index represents the node and value representsits parent
 ...
 *  lecture, except that you should not assume that the arguments to setunion
 ...
 */
   ...
   *
   * @param  elem [description]
   * @return      This function should return the number of nodes in the up-tree
