...
#include "cs225/PNG.h" // PNG, HSLAPixel
#include <vector>
...
public:
   ...
   *
   * This function will be very helpful in solving the maze. It will also be
   * used by the grading program to verify that your maze is a tree that
   ...
   * where height and width were the arguments to makeMaze. Blacken the entire
   * topmost row and leftmost column of pixels, except the entrance (1,0)
   * through (9,0). For each square in the maze, call its maze coordinates (x,y).
   * If the right wall exists, then blacken the pixels with coordinates
   * ((x+1)*10,y*10+k) for k from 0 to 10. If the bottom wall exists, then
   ...
   * a trail of 11 red pixels in the given direction. If the first step is
   ...
   *
   * Make the exit by undoing the bottom wall of the destination square: call
   * the destination maze coordinates (x,y), and whiten the pixels with
   ...
   * data before doing so. You will start with a square grid (like graph paper)
   ...
   * other member functions. setWall should not prevent cycles from occurring,
   * but should simply set a wall to be present or not present. Our tests will
   * call setWall to copy a specific maze into your implementation.
   *
   * @param x      The x coordinate of the current cell
   * @param y      	The y coordinate of the current cell
   * @param dir    	Either 0 (right) or 1 (down), which specifies which wall to
   * set (same as the encoding explained in canTravel). You only need to support
   * setting the bottom and right walls of every square in the grid.
  ...
  /**
   * My own function used to the part 3 creation part
   * @return the png that is created
   */
  PNG* drawCreativeMaze();
private:
  int find_loc(int x, int y) const;
  int height_; // store the given maze dimension
  int width_; // store the given maze dimension
  vector<bool> flo_wal; // whether down wall is broken or not
  vector<bool> rig_wal; // whether right wall is broken or not
};
