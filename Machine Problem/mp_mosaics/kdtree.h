...
class KDTree
{
  ...
  public:
     ...
     * @param sortcurr_ Dimension these points are being compared in.
     * @return A boolean value indicating whether the first Point is smaller
     *  than the second Point in the sortcurr_ dimension.
     */
     ...
     * reference Point. Takes three points: target, optimal, and
     ...
     *     Point<3> optimal1(1, 3, 2);
     *     Point<3> possibleBest1(2, 4, 4);
     *     Point<3> optimal2(1, 3, 6);
     *     Point<3> possibleBest2(2, 4, 4);
     *     Point<3> optimal3(0, 2, 4);
     *     Point<3> possibleBest3(2, 4, 6);
     *     cout << shouldReplace(target, optimal1, possibleBest1) << endl; // true
     *     cout << shouldReplace(target, optimal2, possibleBest2) << endl; // false
     *     cout << shouldReplace(target, optimal3, possibleBest3) << endl;
     *      // based on operator<, this should be false
     *
     * @todo This function is required for MP 5.1.
     * @param target The Point we want to be close to.
     * @param optimal The Point that is currently our closest Point
     *    to target.
     * @param potential Our Point that is a candidate to replace
     *    optimal (that is, it may be closer to target than
     *    optimal).
     * @return A boolean value indicating whether potential is closer
     *  to target than optimal. Ties should be broken with
     *  Point::operator<().
     */
    bool shouldReplace(const Point<Dim>& target, const Point<Dim>& optimal,
  ...
  private:
    ...
    KDTreeNode * quick_sort(int , int , int);
    double ouqi(const Point<Dim>& one, const Point<Dim>& other) const;
    double ouqi1(const Point<Dim>& one, const Point<Dim>& other,int r) const;
    vector<Point<Dim>> t;
    void choosefast(int,int, int, int);
    int partition(int , int , int , int);
    void makecopy(const KDTree<Dim>& other);
    void makeclear(KDTreeNode* root);    
    void helper_nearest( Point<Dim> &optimal,KDTreeNode* now,
