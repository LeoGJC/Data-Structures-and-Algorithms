                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    TreeNode* tomove = singleQueue.front();
    TreeNode* toleave = mergeQueue.front();
    if (tomove->freq.getFrequency() > toleave->freq.getFrequency()) {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    unsigned t = 0;
    while(t < frequencies.size()){
        Frequency add = frequencies[t];
        TreeNode* leafNode = new TreeNode(add);
        singleQueue.push(leafNode);
        t++;
    }
    while(singleQueue.size()+mergeQueue.size() > 1){
      TreeNode* tomove = removeSmallest(singleQueue, mergeQueue);
      TreeNode* toleave = removeSmallest(singleQueue, mergeQueue);
      if (toleave == NULL) {
        TreeNode* aggregate = new TreeNode(tomove->freq);
        mergeQueue.push(aggregate);
      }
      TreeNode* aggregate = new TreeNode(tomove->freq.getFrequency() + toleave->freq.getFrequency());
      aggregate->left = tomove;
      aggregate->right = toleave;
      mergeQueue.push(aggregate);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  while (bfile.hasBits() != false) {
    ...
    if(bfile.getNextBit()){
      TreeNode* empty = new TreeNode(Frequency(bfile.getNextByte(), 0));
      return empty;
    }
    else{
      TreeNode* middle = new TreeNode(0);
      middle->left = readTree(bfile);
      middle->right = readTree(bfile);
      return middle;
    }
  }
