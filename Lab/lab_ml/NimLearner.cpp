...
std::vector<Edge> NimLearner::playRandomGame() const {
 ...
 /* Your code goes here! */
  ...
  Vertex begin = startingVertex_;
  Vertex po;
  do{
    vector<Vertex> near_ = g_.getAdjacent(begin);
    po = near_[rand() % near_.size()];
    path.push_back(g_.getEdge(begin, po));
    begin = po;
  }
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
 /* Your code goes here! */
  int ele = (int)path.size() - 1;
  int convert = 1;
  do {
    Edge peri = path[ele];
    g_.setEdgeWeight(peri.source, peri.dest, g_.getEdgeWeight(peri.source, peri.dest) + convert);
    ele--;
    convert /= -1;
  } while (ele >= 0);
}
