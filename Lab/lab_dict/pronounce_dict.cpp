...
{
    /* Your code goes here! */
    string number_1 = word1;
    string number_2 = word2;
    // captilize
    std::transform(number_1.begin(), number_1.end(), number_1.begin(), ::toupper);
    std::transform(number_2.begin(), number_2.end(), number_2.begin(), ::toupper);
    
    if (dict.find(number_1) == dict.end() || dict.find(number_2) == dict.end()) { // should consider this case, otherwise will not find and assign the second value
        return false;
    }
    if (dict.find(number_1)->second.size() != dict.find(number_2)->second.size()) {
        return false;
    }
    size_t i =0;
    do {
        if (dict.find(number_1)->second[i] != dict.find(number_2)->second[i]) {
            return false;
        }
        i++;
    }
    while (i < dict.find(number_1)->second.size());
